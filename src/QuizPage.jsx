  ...
  return (
    <div className="flex justify-center items-center p-20">
      <div className="card w-full max-w-xl bg-base-100 shadow-md">
        {currentStep === "info" && (
          <QuizInfo {...quizData} onStart={handleStart} />
        )}
...
function QuizInfo({
  ...
  const [remainingTime, setRemainingTime] = useState(
    Math.max((scheduledTime - new Date()) / 1000, 0)
  );
  ...
  return (
    <>
      ...
      <div className="card-body">
        ...
        {/* First line: Questions, Time, Difficulty, Type */}
        <div className="flex flex-wrap gap-2 mb-2">
          <span className="badge badge-outline">{numQuestions || 3} Questions</span>
          <span className="badge badge-outline">{Math.round(timeLimit / 60)} min</span>
          <span className="badge badge-outline">{difficulty}</span>
          <span className="badge badge-info">{type || "General"}</span>
        </div>
        ...
        {/* Third line: Quiz status + User status */}
            ...
            {["days", "hours", "min", "sec"].map((unit, i) => (
              <div key={unit} className="flex flex-col p-2 bg-neutral rounded-box text-neutral-content">
                <span className="countdown font-mono text-2xl">
                  <span style={{ "--value": countdownValues[i] }}></span>
                </span>
                {unit}
              </div>
            ))}
